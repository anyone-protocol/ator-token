...
async function main() {
  ...
  console.log(`Operator ${deployer.address} calling enableTrading(0) on token contract ${atorContractAddress}`)
  const enableTradingResult = await atorToken.enableTrading(0)
  console.log(`enableTrading(0) tx ${enableTradingResult.hash} waiting for confirmation...`)
  await enableTradingResult.wait()
  console.log(`enableTrading(0) tx ${enableTradingResult.hash} confirmed!`)
}
